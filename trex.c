/** External library, not written for Springbok */
...
TRexBool trex_matchcontinous(TRex* exp, const TRexChar* text_begin, const TRexChar* text_end, const TRexChar** out_end)
{
	const TRexChar* res = NULL;
	exp->_bol = text_begin;
	exp->_eol = text_end;
	exp->_currsubexp = 0;
	res = trex_matchnode(exp,exp->_nodes,text_begin,NULL);
	if(res == NULL)
		return TRex_False;
	return TRex_True;
}
